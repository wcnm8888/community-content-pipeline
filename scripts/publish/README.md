...
- `Draft`: requires `out/article-review-latest.json` to have human review status `approved` and each requested platform in `out/platform-review-latest.json` to have platform review status `approved`, then fills the article and platform fields where selectors are confidently found, uploads the requested cover file, selects AI declarations when visible, and saves draft when a safe draft button is found. It never clicks final publish.
- `Publish`: has the same article/platform review gates, runs the Draft path first, and before any final publish click the terminal requires `YES` for that platform. Draft mode is the recommended mode when final publishing must remain manual.

After article approval, the local worker generates the platform versions and creates `out/platform-review-latest.json`. Review each platform in the platform pack page before using Draft or Publish mode.
